...
const bday = new Date('Oct 31, 1995')
console.log(bday)
...
console.log(age / 1000 / 60, '<- Age in m')
console.log(age / 1000 / 60 / 60, '<- Age in m')
console.log(age / 1000 / 60 / 60 / 24, '<- Age in d')
console.log(age / 1000 / 60 / 60 / 24 / 365.25, '<- Age in y')
...
function consecutiveDates(date, repeat, offset) {
  ...
  for (let i = 0; i < repeat; i++) {
    datesList.push(new Date(nextDate)); 
...
function consecutiveDatesUnit(date, repeat, offset, unit = 'day') {
  const datesList = [];
  const nextDate = new Date(date);
...
console.log(consecutiveDatesUnit(new Date(2019, 0, 1), 4, 3, 'day'));
console.log(consecutiveDatesUnit(new Date(2019, 0, 1), 3, 1, 'year'));
...
function orderDates(dates) {
  ...
  dates.forEach(date => {
    if (date.getDate() === currentDate.getDate()) {
      obj.present.push(date);
    } else if (date < currentDate) {
      obj.past.push(date);
    } else {
      obj.future.push(date);
    }
  });
...
// DONE Stretch: Return an object containing three keys each holding an array of dates. The keys are: 
...
// DONE Stretch Goal: Return a human readable string: 
...
function whensYourParty(date, year) {
  const futureBday = new Date(year, date.getMonth(), date.getDate())
  const days = ['Sun', 'Mon', 'Tues', 'Wed', 'Thur', 'Fri', 'Sat']
  return days[futureBday.getDay()]
}

console.log(whensYourParty(bday, 2026))

// DONE Stretch Goal: Return an array listing all 
// the days when your birthday occured since 
// you were born. 

function listBdays(bday) {
  const datesList = [];
  const currentDate = new Date()
  ...
  for (let i = 0; i < yearsSinceBirth; i++) {
    const currentBday = new Date(birthDate)
    currentBday.setFullYear(birthDate.getFullYear() + i)
    const days = ['Sun', 'Mon', 'Tues', 'Wed', 'Thur', 'Fri', 'Sat']
    const weekDay = days[currentBday.getDay()]
    datesList.push(weekDay)
  }

  return datesList
}

console.log(listBdays(bday))